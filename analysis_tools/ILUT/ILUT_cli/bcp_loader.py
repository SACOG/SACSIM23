...
from format_for_bcp import fileChecker
...
class BCP():
    ...
    def check_table_size(self, tbl_name, input_file):
        """Get count of rows in loaded table (tbl_name). If zero, it probably means
        that the user specified a UNC path for a remote machine. To remedy,
        The user must map the remote the machine to a drive letter name."""

        sql_getcnt = f"SELECT COUNT(*) FROM {tbl_name}"

        with pyodbc.connect(self.str_conn_info, autocommit=True) as conn:
            cursor = conn.cursor()
            cursor.execute(sql_getcnt)
            ...
            if rowcnt == 0:
                curr_dir = os.getcwd()
                errmsg_norows = f"""
                WARNING: Table {tbl_name} has {rowcnt} rows in it. 
                If the input file ({input_file}) is not entered as a full, absolute file path,
                please ensure it is a full, absolute path instead of just the file name.
                """
                raise Exception(errmsg_norows)
            else:
                pass                
    ...
    def create_sql_table_from_file(self, file_in, str_create_table_sql, tbl_name,
        ...
        #------------ensure that correct end-of-line (EOL) characters and no leading commas in ESRI-exported CSVs
        csv_obj = fileChecker(file_in, overwrite=True)
        csv_obj.check_eol_char()
        csv_obj.leading_comma_warn()
        if csv_obj.file_changed:
            csv_obj.export_to_file() # will simply replace CSV file if needed
        ...
        try:
            ...
            print("BCP load fail. Things to try or check:\n" \
                  ...
                  "5 - Make sure you are specifying the correct delimiter type\n" \
                  "6 - If you loaded a TXT file with non-comma delimiters, you need to specify the delimiter type." \
                      "If no delimiter specified, a comma delimiter is assumed.\n\n")
            
            trace()
            sys.exit(1)
        finally:
            self.check_table_size(tbl_name, file_in) # confirm that data actually loaded from file to table.        
...
if __name__ == '__main__':
    ...
    #----------------TEST PARAMS---------------------
    data_folder = r'C:\Users\dconly\GitRepos\SQL-tools\bulk_loader\test_inputs' # r"Q:\SACSIM23\Parcel\2020_baseline_v3\to_SACSIM23\Post_Allocation_results"
    # test_file_in = os.path.join(data_folder, "eto.csv" ) # r"P:\NPMRDS data\Raw Downloads\DynamicData_15Min\2020\test_trucks_Dec1_2020_yuba\test_trucks_Dec1_2020_yuba.csv"  # r"P:\NPMRDS data\Python\Data Prep\LoadRawData\samplett.csv"
    test_file_in = os.path.join(data_folder, "test_inputs.csv" ) # r"P:\NPMRDS data\Raw Downloads\DynamicData_15Min\2020\test_trucks_Dec1_2020_yuba\test_trucks_Dec1_2020_yuba.csv"  # r"P:\NPMRDS data\Python\Data Prep\LoadRawData\samplett.csv"
    
    # sql_to_run = r"C:\Users\dconly\GitRepos\SQL-tools\bulk_loader\testing_sql_script\raw_eto_2020.sql"  # r"P:\NPMRDS data\Python\Data Prep\LoadRawData\qry\create_tt_table_no_tstamp.sql"  # r"P:\NPMRDS data\Python\Data Prep\LoadRawData\qry\create_tt_table.sql" # create_tt_table_chartime
    sql_to_run = r"C:\Users\dconly\GitRepos\SQL-tools\bulk_loader\testing_sql_script\test_input_tbl.sql"  # r"P:\NPMRDS data\Python\Data Prep\LoadRawData\qry\create_tt_table_no_tstamp.sql"  # r"P:\NPMRDS data\Python\Data Prep\LoadRawData\qry\create_tt_table.sql" # create_tt_table_chartime
    data_first_row = 2
    test_tbl_name = "TEST_input_frombcp"
    ...
    with open(sql_to_run, 'r') as f_sql_in:
        ...
        formatted_sql_maketbl = raw_sql_maketbls.format(test_tbl_name)
    ...
    tbl_loader.create_sql_table_from_file(test_file_in, formatted_sql_maketbl, test_tbl_name,
