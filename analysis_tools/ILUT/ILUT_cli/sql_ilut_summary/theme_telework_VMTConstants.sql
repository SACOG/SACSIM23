...
IF OBJECT_ID('{telewk_outtbl}', 'U') IS NOT NULL 
DROP TABLE {telewk_outtbl}; --output table name. Delete if it exists
...
SELECT
	...
	END AS telework_ind
	INTO #person_tw
FROM {raw_personday} pd --raw person_day table
...
INTO {telewk_outtbl} --temporary table with telework data
FROM #tw_vmt
GROUP BY hhparcel
