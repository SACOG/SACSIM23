...
IF OBJECT_ID('{cvixxi_outtbl}', 'U') IS NOT NULL 
DROP TABLE {cvixxi_outtbl}; --ilut person output table name
...
INSERT INTO #trips_jobs
	...
	FROM {raw_parcel} --raw scenario parcel file
...
INSERT INTO #trips_jobs_TAZ
	...
	FROM {raw_cveh} cv --raw cveh table
...
INSERT INTO #pcl_cveh_data
	SELECT
		...
		CASE WHEN tjt.tripXjob_3axTAZ = 0 THEN 0
			...
			END AS CV3_VHT_P
	FROM {raw_cveh} cv --raw cveh table
		LEFT JOIN {raw_parcel} p --raw scenario parcel file
			ON cv.I = p.taz_p
...
INSERT INTO #parcel_pop1
	SELECT
		...
		SUM(CASE
			...
			END) AS pop_pcl
	FROM {raw_parcel} p --raw parcel table
		LEFT JOIN {raw_hh} hh --raw hh table
...
INSERT INTO #rad_pop_aggreg
	...
	FROM {raw_parcel} p ----raw parcel table
		...
		LEFT JOIN {taz_rad_table} tr  --taz-rad lookup table
			ON p.TAZ_p = tr.taz
	GROUP BY tr.RAD
...
INSERT INTO #taz_ixxi_vehdata
	...
	FROM {raw_ixxi} ix --raw ixxi table
		LEFT JOIN {taz_rad_table} tr --taz-rad lookup table
			ON ix.I = tr.TAZ
...
INSERT INTO #parcel_pop_emp
	...
	FROM {raw_parcel} p --raw scenario parcel file
...
INSERT INTO #parcel_ixxi_data
	...
	FROM {taz_rad_table} tr --tazrad lookup table
		LEFT JOIN #rad_ixxi_rates rir --left join to ensure all RADs initially included
			ON tr.RAD = rir.RAD
		JOIN {raw_parcel} p --raw scen parcel table; do inner join because we don't want RADs/TAZs without parcels returned
			ON tr.TAZ = p.taz_p
...
INTO {cvixxi_outtbl} --cveh-ixxi output ilut table
