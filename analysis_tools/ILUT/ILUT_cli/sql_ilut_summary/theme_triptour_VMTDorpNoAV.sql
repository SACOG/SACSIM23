/*
Name: theme_triptour_VMTDorpNoAV.sql
Purpose: Summarize trip and tour data at parcel level--DOES NOT INCLUDE IXXI OR CVEH. THESE ARE IN SEPARATE SCRIPT
	...
	FOR VMT - this script uses DORP (driver or passenger) value. If dorp = driver, then include the VMT. Otherwise don't include VMT.
...
INSERT INTO #vehicle_trips_temp
	SELECT
		...
		trip.tour_id,
		CASE 
			WHEN trip.trip_mode IN (3,4,5,9) AND trip.trip_dorp = 1 --drop = 1 means person making the trip is driver.
...
--	FROM {raw_ixworkerfraxn} --raw worker fraction x taz table
