...
IF OBJECT_ID('{person_outtbl}', 'U') IS NOT NULL 
DROP TABLE {person_outtbl}; --ilut person output table name
...
INSERT INTO #temp_person
	SELECT
		...
		pop.relate,
		per.pwtyp
	FROM {pop_table} pop --POPULATION2016
		JOIN {raw_person} per --raw_person_testscen
...
INSERT INTO #temp_wkrs_x_parcel	
	...
	FROM {raw_parcel} rp --raw parcel table
...
SELECT
	...
	SUM(CASE WHEN tp.pwtyp > 0 THEN 1 ELSE 0 END) AS WKRS_tot, --number of workers living on the parcel, regardless of whether full or part time
	...
	SUM(CASE WHEN tp.ethc = 4 THEN 1 ELSE 0 END) AS PPASN,
	SUM(CASE WHEN tp.ethc = 5 THEN 1 ELSE 0 END) AS PPOTH,
...
INTO {person_outtbl} --output theme table name
