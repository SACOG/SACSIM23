...
class HQTransitStops:
    def __init__(self, in_tranline_txt, keep_all, hf_th_mins=15):
    ...
    def get_node_svc_data(self, df_lnd_data, node_id):
        ...
        df_node_hifreq = df_node.loc[(df_node[f_color] != self.color_brt) \
                                    & (df_node[f_mode] != self.mode_rail) \
                                    & (~df_node[f_name].isin(self.rail_exceptions_trimmed) \
    ...
    def make_hq_stop_df(self):
        ...
        if not self.keep_all:
            self.final_df = self.final_df.loc[(self.final_df[self.k_rail_lines] != '') \
                | (self.final_df[self.k_brt_lines] != '') \
                | (self.final_df[self.k_cnt_hifreqlines] > 1)]
    ...
    def export_to_esri_fc(self, node_dbf, output_gdb):
        ...
        sedf_out = pd.DataFrame.spatial.from_xy(sedf_join, x_column=f_x, y_column=f_y, sr=sr_sacog)
        ...
        sedf_out.spatial.to_featureclass(os.path.join(output_gdb, out_fc))

        user_msg = f"""Success! Output file is {os.path.join(output_gdb, out_fc)}
    ...
    def export_to_csv(self, output_dir):
        ...
        self.final_df.to_csv(os.path.join(output_dir, out_csv), index=False)


if __name__ == '__main__':
    in_tranline_lin = r"Q:\SACSIM23\Network\SM23GIS\SHP\UD4H Data\2020_tranline_20221216.lin" 
    out_gdb = r'Q:\SACSIM23\Network\SM23GIS\MajorTransitStops.gdb'
    ss_hwy_node_dbf = r"\\win10-model-4\D\SACSIM23\2020_RemoveTrnPulsNodes\run_2020_base_noPulseNodes\2020_hwy_node.dbf"

    hf_threshold_mins = 17.9 # max headway in mins to qualify as hi-freq--though in some cases may want to nudge up to account for "shouldering"
    ...
    hqts.export_to_esri_fc(node_dbf=ss_hwy_node_dbf, output_gdb=out_gdb)